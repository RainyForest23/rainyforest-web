...
/** Vendored shadcn output: regenerated by the CLI, so hand-edits would not survive. */
export const VENDOR_DIR = 'components/primitives/'
...
export function findViolations(path: string, content: string): Violation[] {
  if (path === TOKENS_FILE || path.startsWith(VENDOR_DIR)) return []
